...
class RepReadingPipeline(Pipeline):
    ...
    def _get_hidden_states(
        ...
        for layer in hidden_layers:
            hidden_states = outputs['hidden_states'][layer] # len(outputs['hidden_states']): 33
            ...
            # print(1, hidden_states_layers[layer].device)
    ...
    def _sanitize_parameters(self, 
        ...
        forward_params['precomputed_hidden_state'] = precomputed_hidden_state
        self.start_idx = 0
        self.end_idx = 0
    ...
    def _forward(self, model_inputs, rep_token, hidden_layers, rep_reader=None, component_index=0, which_hidden_states=None, precomputed_hidden_state=None):
        """
        Args:
        ...
        self.end_idx += bs
        if precomputed_hidden_state == None:
            with torch.no_grad():
            ...
            hidden_states = {}
            for layer_idx, tensor in precomputed_hidden_state.items():
                hidden_states[layer_idx] = tensor[self.start_idx: self.end_idx]
    ...
    def _batched_string_to_hiddens(self, train_inputs, rep_token, hidden_layers, batch_size, which_hidden_states, **tokenizer_args):
        """
        train_inputs->batched_train_inputs->_forward
            -> [output, output, ...]
        """
        ...
        for hidden_states_batch in hidden_states_outputs:
            for layer in hidden_states_batch:
                """
                hidden_states_batch[layer] : [1, 5120]
                """
                # print(2, hidden_states_batch[layer].device)
    ...
    def _validate_params(self, n_difference, direction_method):
        ...
        if direction_method == 'cluster_mean':
            assert n_difference == 0, "n_difference must be 0 for clustermean"
    ...
    def get_directions(
        ...
        direction_finder.directions = direction_finder.get_rep_directions(
            ...
            train_choices=train_labels, save_hidden_states=save_hidden_states)
