...
class RepReader(ABC):
        ...
        if self.needs_hiddens and hidden_states is not None and len(hidden_states) > 0:
            for layer in hidden_layers:    
                ...
                for component_index in range(self.n_components):
                    ...
                    # projected_scores len 3383 tensor([-1.2904, -1.3167], device='cuda:0')
                    # train_choices 3383 [False, True]
                    outputs_min = []
                    idx = 0
                    for o, label in zip(projected_scores, train_choices):
                        try:
                            if min(o) == o[label]:
                                outputs_min.append(1)
                            else:
                                outputs_min.append(0)
                            idx += 1
                        except:
                            import pdb
                            pdb.set_trace()
    ...
    def transform(self, hidden_states, hidden_layers, component_index):
        ...
        for layer in hidden_layers:
            ...
            dtype('float32')
            """
            # print(3, layer_hidden_states.device)
...
class PCARepReader(RepReader):
    ...
    def get_rep_directions(self, model, tokenizer, hidden_states, hidden_layers, **kwargs):
        ...
        """
        for layer in hidden_layers:
            H_train = hidden_states[layer] # (150, 5120)
    ...
    def get_signs(self, hidden_states, train_labels, hidden_layers):
        ...
        for layer in hidden_layers:
            ...
            for component_index in range(self.n_components):
                ...
                """
                i = 0
                islice(lst, start=0, end=2)
                transformed_hidden_states:[0,1,2,3,...]
                ↓
                [[0,1],[2,3],...]
                """
                ...
                """
                pca_outputs_max:0.37333333333333335
                pca_outputs_min:0.6266666666666667
                -:-0.25333333333333335
                np.sign:>0=1,=0=0,<0=-1
                解释：
                随机找到的这个主方向，会偏哪类样本多一点？
                我们用所有pair中的正样本来检查（index(1)）
                结果显示，这些正样本的0.37在主成分的max一边（同向），0.62在主成分的min一边（反向），说明主成分的方向是负样本方向
                此时，sign=-1。这种情况下，pair中，与主成分反向的，或者点积分数更小的，判断为正
                可通过，
                `eval_func = min if sign == -1 else max
                eval_func(H) == H[0]`
                来判断正样本(H[0])是否被预测为正(eval_func(H))
                """
...
class ClusterMeanRepReader(RepReader):
    ...
    def get_rep_directions_2(self, model, tokenizer, hidden_states, hidden_layers, **kwargs):
        ...
        # train_choices = train_choices.reshape(train_choices.shape[0]*train_choices.shape[1])
        ...
        # neg_class = train_choices == 0
        # pos_class = train_choices == 1
        neg_class = np.where(train_choices == 0)[1] == 0
        pos_class = np.where(train_choices == 1)[1] == 0
        """
        0-1
        如果一个pair的label的0位置是0，则为0-1,负例

        """

        directions = {}
        for layer in hidden_layers:
            H_train = np.array(hidden_states[layer]) # (150, 5120)
    ...
    def get_rep_directions(self, model, tokenizer, hidden_states, hidden_layers, save_hidden_states=False, **kwargs):
        ...
        for layer in hidden_layers:
            ...
            H_pos_mean = H_train[pos_class].mean(axis=0, keepdims=True)
            H_neg_mean = H_train[neg_class].mean(axis=0, keepdims=True)
            saved_hidden_states[layer] = {
                "pos": H_train[pos_class],
                "neg": H_train[neg_class]
            }
            directions[layer] = H_pos_mean - H_neg_mean
        if save_hidden_states:
            self.saved_hidden_states = saved_hidden_states
        return directions
